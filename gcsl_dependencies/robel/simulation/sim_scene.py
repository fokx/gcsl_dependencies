...
from gcsl_dependencies.robel.simulation.renderer import Renderer
...
class SimScene(metaclass=abc.ABCMeta):
    ...
    def create(*args, backend: Union[SimBackend, int], **kwargs) -> 'SimScene':
        ...
        if backend == SimBackend.MUJOCO_PY:
            from gcsl_dependencies.robel.simulation import mjpy_sim_scene  # type: ignore
            return mjpy_sim_scene.MjPySimScene(*args, **kwargs)
        elif backend == SimBackend.DM_CONTROL:
            from gcsl_dependencies.robel.simulation import dm_sim_scene  # type: ignore
